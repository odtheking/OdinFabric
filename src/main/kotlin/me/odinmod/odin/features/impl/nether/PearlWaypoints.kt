    ...
    fun onRender(event: RenderEvent.Last) {
        ...
        getOrderedLineups(mc.player?.blockPos ?: return).forEach { (lineup, color) ->
            lineup.startPos.forEach {
                if (presetWaypoints) event.context.drawWireFrameBox(Box(it), color.withAlpha(if (!closest && hideFarWaypoints) 0.25f else 1f), if (!closest && hideFarWaypoints) 4f else 6f)
            }
