...
import me.odinmod.odin.OdinMod.scope
...
import me.odinmod.odin.utils.hasBonusPaulScore
import me.odinmod.odin.utils.noControlCodes
import me.odinmod.odin.utils.romanToInt
import me.odinmod.odin.utils.skyblock.dungeon.DungeonUtils.getDungeonTeammates
    ...
    fun onPacket(event: PacketEvent.Receive) {
        when (event.packet) {
            is PlayerListS2CPacket -> {
                if (event.packet.actions.none { it.equalsOneOf(PlayerListS2CPacket.Action.UPDATE_DISPLAY_NAME, PlayerListS2CPacket.Action.ADD_PLAYER) }) return
                val tabListEntries = event.packet.entries?.mapNotNull { it.displayName?.string }?.ifEmpty { return } ?: return
            ...
            is PlayerListHeaderS2CPacket -> {
                Blessing.entries.forEach { blessing ->
                    blessing.regex.find(event.packet.footer?.string ?: return@forEach)?.let { blessing.current = romanToInt(it.groupValues[1]) }
                }
            }
            ...
            is GameMessageS2CPacket -> {
                ...
                deathRegex.find(message)?.let { match ->
                    dungeonTeammates.find { teammate -> teammate.name == (match.groupValues[1].takeUnless { it == "You" } ?: mc.player?.name?.string) }?.deaths?.inc()
                }
