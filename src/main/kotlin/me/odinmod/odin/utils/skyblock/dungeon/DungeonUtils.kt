...
object DungeonUtils {
    ...
    inline val neededSecretsAmount: Int
        get() =
            DungeonListener.floor?.let { ceil(
        ...
        for (line in tabList) {
            ...
            previousTeammates.add(
                DungeonPlayer(
                    ...
                    mc.networkHandler?.getPlayerListEntry(name)?.skinTextures?.texture ?: Identifier.of("textures/entity/steve.png"),
    ...
    fun isSecret(state: BlockState, pos: BlockPos): Boolean {
        return when {
            ...
            state.block is SkullBlock -> (mc.world?.getBlockEntity(pos) as? SkullBlockEntity)?.owner?.id?.toString().equalsOneOf(WITHER_ESSENCE_ID, REDSTONE_KEY)
            else -> false
        }
    }

//    fun Room.getRelativeCoords(pos: Vec3d) = pos.subtract(x = clayPos.x, z = clayPos.z).rotateToNorth(rotation)
//    fun Room.getRealCoords(pos: Vec3d) = pos.rotateAroundNorth(rotation).addVec(x = clayPos.x, z = clayPos.z)
