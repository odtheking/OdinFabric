package me.odinmod.odin.utils.skyblock.dungeon
...
import me.odinmod.odin.utils.skyblock.dungeon.DungeonListener.inBoss
import me.odinmod.odin.utils.skyblock.dungeon.tiles.*
...
//                logger.error("Error reading room data", e)
            ...
            room.roomComponents.any { component ->
                BlockPos(component.x + rotation.x, roomHeight, component.z + rotation.z).let { blockPos ->
                    mc.world?.getBlockState(blockPos)?.block == Blocks.BLUE_TERRACOTTA && (room.roomComponents.size == 1 || horizontals.all { facing ->
                        mc.world?.getBlockState(blockPos.add((if (facing.axis == Axis.X) facing.offsetX else 0), 0, (if (facing.axis == Axis.Z) facing.offsetZ else 0)))?.block?.equalsOneOf(Blocks.AIR, Blocks.BLUE_TERRACOTTA) == true
    ...
    private fun findRoomComponentsRecursively(vec2: Vec2, cores: List<Int>, visited: MutableSet<Vec2> = mutableSetOf(), tiles: MutableSet<RoomComponent> = mutableSetOf()): MutableSet<RoomComponent> {
        ...
        horizontals.forEach { facing ->
            findRoomComponentsRecursively(Vec2(vec2.x + ((if (facing.axis == Axis.X) facing.offsetX else 0) shl ROOM_SIZE_SHIFT), vec2.z + ((if (facing.axis == Axis.Z) facing.offsetZ else 0) shl ROOM_SIZE_SHIFT)), cores, visited, tiles)
        }
        return tiles
    }
    ...
    fun getCore(vec2: Vec2): Int {
        ...
        val height = chunk.getHeightmap(Heightmap.Type.WORLD_SURFACE).get(vec2.x and 15, vec2.z and 15).coerceIn(11..140)
    ...
    private fun getTopLayerOfRoom(vec2: Vec2): Int {
        ...
        return if (mc.world?.getBlockState(BlockPos(vec2.x, height, vec2.z))?.block == Blocks.GOLD_BLOCK) height - 1 else height
    }
