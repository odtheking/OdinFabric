...
object SkyblockPlayer {
    ...
    inline val currentHealth: Int get() = (mc.player?.let { player -> (maxHealth * player.health / player.maxHealth).toInt() } ?: 0)
    var maxHealth: Int = 0
    ...
    fun onPacket(event: PacketEvent.Receive) = with (event.packet) {
        ...
        currentSpeed = floor((mc.player?.getAttributeInstance(EntityAttributes.MOVEMENT_SPEED)?.baseValue?.toFloat() ?: 0f) * 1000f).toInt()
    }
}